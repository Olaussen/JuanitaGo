...
type Context struct {
	...
	Conf       *JuanitaConfig
	CmdHandler *JuanitaCommandHandler
	Sessions   *JuanitaSessionManager
	Youtube    *Youtube
}

func NewContext(discord *discordgo.Session, guild *discordgo.Guild, textChannel *discordgo.Channel,
	user *discordgo.User, message *discordgo.MessageCreate, conf *JuanitaConfig, cmdHandler *JuanitaCommandHandler,
	sessions *JuanitaSessionManager, youtube *Youtube) *Context {
	ctx := new(Context)
	ctx.Discord = discord
	ctx.Guild = guild
